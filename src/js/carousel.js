...
;(function($, window, document, undefined) { // 将window/document等系统变量作为参数传递到插件内部，这样在需要访问这些系统变量的时候就可以访问这些参数，也就是访问局部变量，比访问全局变量速度更快，有少许性能提升
	...
	function Slide (el, opts) {
		// body...
		this.opts = $.extend({}, Slide.DEFAULTS, opts);
	    this.$el = $(el);
	...
	Slide.DEFAULTS = {
		...
		mode: 'slide', // 切换时的动画效果，默认是slide，滑动；可选fade，淡入淡出
		autoPlay: true, // 是否自动切换，默认true
		arrow: true, // 是否显示切换箭头
		button: true, // 是否显示底部切换按钮
		trigger: 'mouseenter', // banner下方按钮切换触发方式，默认mouseenter，可选click
	...
	$.fn.extend({
	    slide: function(opts) {
	        ...
	        return this.each(function() {
	            new Slide(this, opts); // 这里的this指向dom元素
	        })
	    }
	})
